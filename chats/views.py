...
def chat_view(request, car_id):
    ...
    if not chat:
        ...
        chat.participants.add(request.user, car.owner)
    ...
    chat.messages.filter(sender__ne=request.user).update(is_read=True)

    return render(request, 'chats/chat.html', {'chats': chat, 'car': car})
